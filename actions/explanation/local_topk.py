...
def topk_operation(conversation, parse_text, i, **kwargs):
    return_s = ""
    if len(parse_text[i+1])>0 and parse_text[i+1].isdigit():
        id_val = int(parse_text[i+1])
    else:
        id_val = -1
    if id_val<0:
        return "Invalid id, please try again!", 1
    if len(parse_text[i+2])>0 and parse_text[i+2].isdigit():
        k_val = int(parse_text[i+2])
    else:
        k_val = 10
    ...
    for j in range(len(all_model_predictions)):
        ...
        if dataset_name=="boolq":
            ...
            return_s += "<b>Text:</b><br>" + instance_texts[j] + "<br>"
