"""
    Augmenter that apply operation (word level) to textual input based on contextual word embeddings. Based on the NlpAug library: https://github.com/makcedward/nlpaug
"""
...
class CustomContextualWordEmbsAug(WordAugmenter):
...
#     'microsoft/deberta-base'
    ...
    def insert(self, data):
        ...
        new_tokens = self.model.get_tokenizer().decode(ids).split()
        orig_ids = self.model.get_tokenizer().convert_tokens_to_ids(self.model.get_tokenizer().tokenize(data))
        orig_tokens = self.model.get_tokenizer().decode(orig_ids).split()

        return augmented_texts, aug_idxes, new_tokens, orig_tokens
        #if isinstance(data, list):
